...
import { Alert, Switch, View, TextInput, Picker, TouchableHighlight } from 'react-native';
...
/**
 * CreateUserPage is the component where an admin create a new user.
 */
export default class CreateUserPage extends React.Component {
	...
	submitUser = () => {
		...
		// Get the collection
		const userCredentialsCollection = this.getMongoCollection();

        var tempUserId = Math.random() * 1000; // Temporary id Data before Stitch assigns ID server side
		...
		userCredentialsCollection.insertOne(newUserCredential)
			.then(result => {
				console.log("User with id: "+this.state.inputID+" successfully created");
				// Create a new account
				this.createNewAccount();
			})
			.catch(err =>  {
				console.log(`Failed to insert item: ${err}`);
				// Catch any errors, alert user and fail silently so app doesn't crash
				Alert.alert("Failed to add user. User with that id might already exist.");
			});
		
	}
	
	// Function to create new MongoStitch account for user
	createNewAccount() {
		// Create MongoStitch account for new user
		...
		emailPasswordClient.registerWithEmail(this.state.email, this.state.password)
			.then(() => {
				...
				// If successfully added user, alert user and navigate back to admin page.
				Alert.alert("User with id: "+this.state.inputID+" successfully created");
			...
			.catch(err => {
				...
				// Catch any errors, alert user and fail silently so app doesn't crash
				Alert.alert("Failed to add user. Please check that the entered email is valid.");
			});
	}

	// Function to delete an account if the id was valid but the email was invalid
	deleteTempAccount() {
		// Get the collection
		const userCredentialsCollection = this.getMongoCollection();
		...
		// Delete the user
		userCredentialsCollection.deleteOne(query)
			.then(result => {
				console.log("Deleted research_id: "+this.state.inputID);
			})
			.catch(err => {
				console.log("Deleted failed for research_id: "+this.state.inputID)
			})
	}
	...
	render() {
		...
		var PersonalisedActivities = this.state.personalisedActivities.map((activity, index) => {
			return(
			<View key = {index} style={[{width: '33%', flexDirection: 'row', alignItems: 'center', marginBottom: 5}]}>
